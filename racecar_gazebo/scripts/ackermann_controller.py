...
Subscribed Topics:
    /chassisCommand (racecar_msgs.msg.ChassisCommand)
        Chassis command. It contains the vehicle's desired speed and steering
        angle by percentage.
...
class _AckermannCtrlr(object):
    ...
    def __init__(self):
        ...
        # Racecar Properties
        self._max_speed = rospy.get_param('racecar_properties.max_speed', 10.0)
        self._max_steering = rospy.get_param(
            'racecar_properties.max_steering', 0.7)

        self._command_priorities = rospy.get_param(
            "~chassis_command_priorities", [])
        self._command_priorities = sorted(
            self._command_priorities.items(), key=operator.itemgetter(1))
        rospy.loginfo("RacecarGazeboController: Loaded %d chassis commanders", len(
        ...
        self._chassis_cmd_sub = dict()
        self._chassis_cmds = dict()
        for sender, _ in self._command_priorities:
            self._chassis_cmd_sub[sender] = \
                rospy.Subscriber(sender + "/chassis_command", ChassisCommand,
                                 lambda cmd: self.chassis_cmd_cb(sender, cmd), queue_size=1)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if 0.0 < self._cmd_timeout < t - self._last_cmd_time:
                ...
                with self._chassis_cmd_lock:
                    ...
                    for sender, priority in self._command_priorities:
                        # rospy.logwarn("looking for chassis commander %s with priority %d", sender, priority)
                        if sender in self._chassis_cmds:
                            if (rospy.Time.now() - self._chassis_cmds[sender].header.stamp) >= \
                                    rospy.Duration.from_sec(0.2):
                                continue

                            if abs(self._chassis_cmds[sender].steering) <= 1.0 and not found_steering:
                                # rospy.loginfo("%s in control of steering", sender)
                                steer_ang = self._chassis_cmds[sender].steering * self._max_steering
                                steer_ang_vel = 0.0
                                found_steering = True

                            if abs(self._chassis_cmds[sender].throttle) <= 1.0 and not found_throttle:
                                # rospy.loginfo("%s in control of throttle", sender)
                                speed = self._chassis_cmds[sender].throttle * self._max_speed
                                accel = 0.0
                                found_throttle = True
    ...
    def chassis_cmd_cb(self, sender, chassis_cmd):
        """Ackermann driving command callback

        :Parameters:
          chassis_cmd : racecar_msgs.msg.ChassisCommand
            Ackermann driving command.
        """
        with self._chassis_cmd_lock:
            self._chassis_cmds[sender] = chassis_cmd
            self._last_cmd_time = rospy.get_time()
