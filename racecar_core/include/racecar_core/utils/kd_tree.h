...
#ifndef PROJECT_KD_TREE_H
#define PROJECT_KD_TREE_H
...
public:
  ...
  */
  ...
  inline std::vector<PointT> &points() { return points_; }
  ...
  */
  ...
  std::vector<std::vector<PointT>> cluster(double distance, int minPoints = 1, int maxPoints = INT_MAX) {
    ...
    for (int i = 0; i < points_.size(); i++) {
      ...
      {
        // Search for sq_idx
        auto indices = radiusSearch(points_[seed_queue[sq_idx]], distance);
        if (indices.empty())
        {
          sq_idx++;
          continue;
      ...
      if (seed_queue.size() >= minPoints && seed_queue.size() <= maxPoints)
      {
        std::vector<PointT> cluster;
        for(const int j : seed_queue) {
          cluster.push_back(points_[j]);
        }
