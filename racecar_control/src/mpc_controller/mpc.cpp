...
public:
  ...
  int _mpc_steps, _x_start, _y_start, _psi_start, _v_start, _cte_start, _epsi_start, _lage_start, _delta_start, _a_start;
  ...
  FG_eval(Eigen::VectorXd coeffs)
  {
    ...
    _lage_start = _epsi_start + _mpc_steps;
    _delta_start = _lage_start + _mpc_steps;
  ...
  void operator()(ADvector& fg, const ADvector& vars)
  {
    ...
    for (int i = 0; i < _mpc_steps - 1; i++)
    {
      ...
      // predicted value - dynamic model = 0  -> equality constraints
      // y_xt < g(xt) < y_xt1
      // to achieve this
      // border(x0) -> [b_l b_u]
      // calculate y(x0) = (y(x0) - b_l) / (b_u - b_l)
      // 0 < y(x0) < 1
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
  ...
  constraints_upperbound[_v_start] = v; // equality constraints
  constraints_upperbound[_cte_start] = cte;
  constraints_upperbound[_epsi_start] = epsi;

  for (int i = 0; i < _mpc_steps; i++)
  {
    constraints_lowerbound[i+_lage_start] = 0;
    constraints_upperbound[i+_lage_start] = 1;
  }
