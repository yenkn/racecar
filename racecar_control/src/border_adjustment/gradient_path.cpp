...
bool GradientPath::getPath(int* potential, double start_x, double start_y, double goal_x, double goal_y, std::vector<std::pair<float, float> >& path) {
  ...
  int ns = xs_ * ys_;
  std::fill_n(gradx_, 0, ns);
  std::fill_n(grady_, 0, ns);
...
float GradientPath::gradCell(int* potential, int n) {
