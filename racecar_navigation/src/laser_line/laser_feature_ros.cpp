...
LaserFeatureROS::LaserFeatureROS(const std::string &name) :
    nh_local_("~" + name),
...
std::vector<gline> LaserFeatureROS::processScan(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  if (!com_bearing_flag) {
...
void LaserFeatureROS::load_params() {
  ...
  nh_local_.param<std::string>("frame_id", frame_id, "laser_link");
  frame_id_ = frame_id;
